...
<body>
  ...
  <iframe src="{{env('APP_URL')}}/{{env('TELESCOPE_PATH')}}/requests" frameborder="10" id="telescope"></iframe>
  </details>

  <input type="url" placeholder="Link" id="url" value="{{env('APP_URL')}}/api/" required>
  <select id="method">
...
<script>
  ...
  let allInputsDiv = document.getElementById("allInputsDiv")
  let pushInputBarOnIndex = 0;
  ...
  function addInputBar() {
    ...
    oldKeyInp = document.querySelectorAll('.keyInp')[pushInputBarOnIndex];
    newKeyInp = document.querySelectorAll('.keyInp')[pushInputBarOnIndex+1];
    oldKeyInp.removeAttribute('oninput');
    newKeyInp.setAttribute('oninput', 'addInputBar()');
    pushInputBarOnIndex++
    var keyValueDivs = document.getElementsByClassName("key-value-divs")
    Array.from(keyValueDivs).forEach((element, index) => {
      let closeBtn = element.querySelector("span.closeBtn")
      closeBtn.addEventListener("click", () => {
        allInputsDiv.removeChild(element)
      })
    })
  }
